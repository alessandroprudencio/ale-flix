      ...
      <form class="space-y-5" @submit.prevent="handleLogin">
            ...
            <div class="relative">
              ...
              >
                <svg v-if="showPassword" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-4.477-10-10a9.96 9.96 0 012.151-6.026M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-1.299 0-2.544-.293-3.638-.818M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </button>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between text-sm">
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="rememberMe" type="checkbox" class="rounded text-indigo-600 focus:ring-indigo-500" >
              Lembrar de mim
            </label>
        ...
        <div class="flex gap-4">
          <button
            class="flex-1 border border-gray-700 rounded py-2 flex items-center justify-center gap-2 hover:bg-gray-700 transition"
            @click="loginWith('google')"
          >
            <svg fill="#EA4335" height="20" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg">
              <path d="M21.35 11.1h-9.15v2.75h5.28c-.22 1.26-1.45 3.7-5.28 3.7-3.18 0-5.77-2.63-5.77-5.86s2.59-5.86 5.77-5.86c1.81 0 3.02.77 3.72 1.43l2.53-2.45c-1.59-1.48-3.65-2.4-6.25-2.4-5.34 0-9.69 4.31-9.69 9.6s4.35 9.6 9.69 9.6c5.56 0 9.26-3.9 9.26-9.42 0-.64-.07-1.12-.17-1.5z"/>
            </svg>
            Google
          </button>
...
function handleLogin() {
  console.log('Login com', email.value, password.value, rememberMe.value)
  alert('Login executado. Integre aqui sua API.')
}
