<template>
  <div class="group relative rounded-lg overflow-hidden bg-[#161b22] transition-all duration-300 hover:scale-105 hover:shadow-xl"
    ...
    <div class="aspect-[16/9] relative">
      <template v-if="hovered && media.streamUrl">
        <VideoPlayer v-if="media" :src="media.streamUrl" :title="media.title" :poster="media.poster" />
        <!-- <video :src="media.streamUrl" autoplay muted loop playsinline class="w-full h-full object-cover" /> -->
      </template>
      <template v-else>
      ...
      <button
        class="absolute top-2 right-2 p-2 rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70 cursor-pointer">
